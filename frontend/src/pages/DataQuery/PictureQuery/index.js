...
const PictureQuery = () => {
  ...
  const showImg = (record) => {
    ...
    if (imgarr.length > 0) {
      imgarr.forEach((e, index) => {
        imgarr[index] = `http://10.16.32.172:8222/oss-access/download?fileName=${e}&&path=${record.path_md5}`
      })
      setImgList(imgarr)
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center',
      valueType: 'option',
      render: (text, record) => [<a key="1" onClick={() => showImg(record)}> 查看</a>]
    },
  ]
  return (
    <PageHeaderWrapper>
      <ProTable
        ...
        request={async (params = {}) => {
          ...
          if (list.length > 0) {
            list.forEach(e => e['copyLink'] = `wget_pictures.sh ${e.path_md5} ${e.package_name}`)
          }
          return {
            data: list,
            page: params.current,
